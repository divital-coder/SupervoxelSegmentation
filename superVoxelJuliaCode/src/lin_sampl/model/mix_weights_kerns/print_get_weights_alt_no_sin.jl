...
function waves_accum_shared_mem(l_relu_weight,channel,param_set_idx,param_tensor
    ...
    # Sigmoid activation
    (1.0 / (1.0 + exp(-(
        shared_mem[$c2,$x2,$y2,$z2] * $param_tensor[1,$cpx,$cpy,$cpz,$xp,$yp,$zp,$channel,$param_set_idx] + 
    ...
    (shared_mem[$c2,$x2,$y2,$z2] * $param_tensor[3,$cpx,$cpy,$cpz,$xp,$yp,$zp,$channel,$param_set_idx]) /
    (1.0 + abs(shared_mem[$c2,$x2,$y2,$z2] * $param_tensor[3,$cpx,$cpy,$cpz,$xp,$yp,$zp,$channel,$param_set_idx]) + 
     $param_tensor[4,$cpx,$cpy,$cpz,$xp,$yp,$zp,$channel,$param_set_idx]) * 
...
Mix of different activation functions - Using sigmoid, tanh, rational, and softplus provides a rich basis of non-linear functions that can approximate many patterns

Bounded outputs - Like sin/cos, the sigmoid and tanh components produce bounded outputs, helping with stability

Same parameter structure - Maintains the 12-parameter structure of the original function

Gradient smoothness - All proposed functions have well-defined, smooth gradients for backpropagation

Computational efficiency - These operations may be more efficient than trigonometric functions on some hardware
...
shared_mem[$c0, $x0, $y0, $z0 ] = (
    ...
    (shared_mem[$c0, $x0, $y0, $z0 ] +(1.0 / (1.0 + exp(-(
    ...
    (1.0 + abs(shared_mem[$c2, $x2, $y2, $z2] * $param_tensor[7, $cpx, $cpy, $cpz, $xp, $yp, $zp, $channel, $param_set_idx]) + 
