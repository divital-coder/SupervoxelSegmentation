...
function loss_functionn(ps, data)
    y_pred=smodel(data, ps)
    return sum(y_pred)
...
optf = OptimizationFunction(loss_functionn, AutoZygote())
# optf = OptimizationFunction(loss_functionn, AutoEnzyme())
# Change AD backend
# optf = OptimizationFunction(loss_functionn, AutoForwardDiff())

# Create typed optimization problem
prob = OptimizationProblem(optf, ps_ca, Array(weights))
...
function main_train(patience::Int=100)
    ...
    fin_loss = 0.0
    print("\n before \n ")
    prob = OptimizationProblem(optf, ps_ca, weights)
