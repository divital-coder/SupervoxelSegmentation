...
def run_julia_command(trial,out_path,restarted):
    ...
    lr_cycle_len=trial.suggest_categorical("lr_cycle_len", [2,5,8,12,20,40,100])
    opt_type=trial.suggest_categorical("opt_type", ["OAdam","Lion","AdamW"])
    closeness_to_discr=trial.suggest_categorical("closeness_to_discr", ["100.0", "1000.0","10000.0","100000.0","1000000.0","10000000.0"])
    
    cmd = f"""DISPLAY={display}
        LEARNING_RATE_START={trial.suggest_float("learning_rate_start", 0.000000001, 0.01, log=True)} \
        LEARNING_RATE_END={trial.suggest_float("learning_rate_end", 0.000000001, 0.001, log=True)} \
        ...
        julia /workspaces/superVoxelJuliaCode_lin_sampl/superVoxelJuliaCode/src/tests/debug_optimazation/simple_model_file.jl"""
    # cmd = f"""DISPLAY={display}
    #     LEARNING_RATE_START={trial.suggest_float("learning_rate_start", 0.000000001, 0.01, log=True)} \
    #     LEARNING_RATE_END={trial.suggest_float("learning_rate_end", 0.000000001, 0.001, log=True)} \
    #     ADD_GRADIENT_ACCUM={str(grad_accum>0).lower()} \
...
# devv=0 python3 /workspaces/superVoxelJuliaCode_lin_sampl/superVoxelJuliaCode/src/tests/debug_optimazation/debug_optuna_hp.py
# devv=1 python3 /workspaces/superVoxelJuliaCode_lin_sampl/superVoxelJuliaCode/src/tests/debug_optimazation/debug_optuna_hp.py
